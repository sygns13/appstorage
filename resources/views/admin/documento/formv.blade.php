<div class="col-md-12" v-if="divFormularioRev && !divFormulario && !divFormularioDetalles">
    <div class="card card-success">
      <div class="card-header">
        <h3 class="card-title">Nueva Versión o Revisión de Documento</h3>
      </div>
                ...
                <div class="col-sm-10">
                  <select class="form-control" style="width: 100%;" v-model="fillobject.area_id" id="cbuarea_id" disabled>
                    ...
                    @foreach ($areas as $dato)
                      <option value="{{$dato->id}}" selected>{{$dato->siglas}} - {{$dato->nombre}}</option> 
                    @endforeach
                  </select>
                </div>
              </div>
          ...
          <div class="card-footer">
            <button style="margin-right:5px;" id="btnGuardar" type="button" class="btn btn-primary" @click="confirmNuevaVersion()"><span class="fas fa-save"></span> Registrar Nueva Versión o Revisión</button>
            <button id="btnClose" type="button" class="btn btn-default" data-dismiss="modal" @click="cerrarModal2()"><span class="fas fa-power-off"></span> Cerrar</button>
          </div>
